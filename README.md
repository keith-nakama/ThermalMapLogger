...
本システムは、8×8マトリックスセンサーによる「熱の分布」の可視化と、SDカードへの「長時間ロギング」という2つの強みを活かし、以下のような用途に活用できます。

- **モータースポーツ・自動車開発**: 走行中のタイヤ表面温度分布やブレーキシステムの熱害確認。単点式の放射温度計では把握できない「温度分布の変化」をリアルタイムに捉え、データとして記録します。
- **電気設備・基板診断**: 動作中のプリント基板や配電盤における異常発熱箇所の特定。多点同時測定により、熱設計の検証や部品の過熱状態を効率的に診断できます。
- **環境・空調の経時的調査（タイムラプス）**: 住宅の断熱欠損（熱漏れ）の検知や空調効率の検証。長時間ロギング機能を活用し、一日を通した室温分布の変化や熱移動の推移を「熱のタイムラプスデータ」として詳細に分析可能です。

![System Overview](docs/images/hardware.jpg)
